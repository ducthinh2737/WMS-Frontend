    ...
    return (
        <div style={{ maxWidth: 600 }}>
            ...
            <Form
                ...
                onFinish={onSubmit}
                initialValues={{
                    type: 0, // mặc định Nguyên vật liệu
                }}
            >
                ...
                <Form.Item name="unitId" label="Đơn vị" rules={[{ required: true }]}>
                    <Select options={units.map(x => ({ label: x.name, value: x.id }))} />
                </Form.Item>

                <Form.Item name="brandId" label="Thương hiệu" rules={[{ required: true }]}>
                    <Select options={brands.map(x => ({ label: x.name, value: x.id }))} />
                </Form.Item>

                <Form.Item name="supplierId" label="Nhà cung cấp" rules={[{ required: true }]}>
                    <Select options={suppliers.map(x => ({ label: x.name, value: x.id }))} />
                </Form.Item>
