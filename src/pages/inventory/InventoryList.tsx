import { Button, message, Select, Typography, Tag, Tooltip } from "antd";
import { useEffect, useState, useCallback } from "react";
import { SearchOutlined, PlusOutlined, HistoryOutlined, CalendarOutlined } from "@ant-design/icons";
...
export default function InventoryList() {
  ...
  const [putawayData, setPutawayData] = useState<{ productId: number; warehouseId: string } | null>(null);
  ...
  const handleOpenPutaway = (record: InventoryDto) => {
    ...
    setPutawayData({ productId: record.productId, warehouseId: targetWarehouseId });
...
/>
      ...
      <WmsTable
        ...
        scroll={{ x: 1300 }} // Tăng độ rộng scroll để chứa thêm cột
        expandable={{
          expandedRowRender: (record: InventoryDto) => (
            <InventoryLockActions record={record} onSuccess={fetchData} />
          ),
          rowExpandable: (record: InventoryDto) => record.availableQuantity > 0 || record.lockedQuantity > 0,
        }}
        columns={[
          {
            title: "Sản phẩm",
            ...
            render: (_: any, record: InventoryDto) => (
              <div>
                ...
                <br />
                <Text type="secondary" style={{ fontSize: "12px" }}>SKU: {record.productCode}</Text>
              </div>
            ),
          },
          {
          ...
          },
          {
            ...
            render: (type?: number) => {
               ...
               const color = type === 1 ? 'gold' : 'default';
          ...
          },
          {
            ...
            width: 100,
            render: (v: number) => (
              <Text type="success" strong>{v.toLocaleString()}</Text>
            ),
          },
          {
            title: "Hành động",
      ...
      <InventoryAdjustForm open={isAdjustOpen} onClose={() => setIsAdjustOpen(false)} onSuccess={() => { setIsAdjustOpen(false); fetchData(); }} />
      <InventoryHistoryModal open={isHistoryOpen} productId={selectedProductId} onCancel={() => setIsHistoryOpen(false)} />
      <PutawayModal
        open={isPutawayOpen}
